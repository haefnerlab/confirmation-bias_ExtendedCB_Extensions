function [wt_pk, err_pk, cs_fig, pk_fig] = plotCSPK(category_infos, sensory_infos, params, pk_hprs, pk_colormap, betarange, clickpts)
...
for i=1:length(sens_pts)
    ...
    wt_pk(i,:) = weights(1:end-1);
    err_pk(i,:) = errors(1:end-1);
    close(tmp_fig);
    ...
    if nargin >= 5 && isequal(pk_colormap, 'beta')
        ...
        weights(1:end-1) = expfit(1) * exp((0:params.frames-1) * expfit(2));
        errors(1:end-1) = nan;
    end
