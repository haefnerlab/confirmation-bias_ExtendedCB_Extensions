function [image_properties, tracker_info, broke_condition, quit] = trialStimuliGabor_pre_gen(Data, image_array, wPtr, tracker_info, settings)
...
left_patch = uint8(squeeze(bpg.genImages(1, Data.template_size, Data.stim_sp_freq_cpp, Data.stim_std_sp_freq_cpp, Data.left_category, 2)) * 64.0 + 127);
right_patch = uint8(squeeze(bpg.genImages(1, Data.template_size, Data.stim_sp_freq_cpp, Data.stim_std_sp_freq_cpp, Data.right_category, 2)) * 64.0 + 127);
[ht, wt] = size(left_patch);
...
for i = 1:Data.number_of_images
    image_texture(i) = Screen('MakeTexture', wPtr, squeeze(image_array(i, :, :)),0,4);
end
...
image_texture(end) = Screen('MakeTexture', wPtr, noise_mask,0,4);
...
Screen('DrawTexture', wPtr, show_left_patch, [], ptbCenteredRect([xc-wt yc], [wt ht]));   % xc, yc indicates the coordinates of the middle of the screen
Screen('DrawTexture', wPtr, show_right_patch, [], ptbCenteredRect([xc+wt yc], [wt ht]));
