function GaborData = ExperimentGabor_pre_gen(GaborData, image_array_pre, frame_array_pre, correct_signals_pre, tr_val, ratios, contrasts)
% image_array_pre = load('PreGenStim.mat');
% disp('data loaded');
...
if isempty(GaborData.model_observer)
    ...
    right_template = uint8(squeeze(bpg.genImages(1, GaborData.template_size, GaborData.stim_sp_freq_cpp, GaborData.stim_std_sp_freq_cpp, GaborData.right_category, 2)) * 64.0 + 127);
    left_template = uint8(squeeze(bpg.genImages(1, GaborData.template_size, GaborData.stim_sp_freq_cpp, GaborData.stim_std_sp_freq_cpp, GaborData.left_category, 2)) * 64.0 + 127);
...
try
    while trial <= GaborData.trials_per_block * GaborData.blocks
        ...
        [image_array, frame_categories, ideal_frame_signals, loc] = ChooseStimulus(GaborData, image_array_pre, frame_array_pre, correct_signals_pre, ratios, contrasts, trial, tr_val);
        GaborData.pre_image_loc(trial) = loc;
        GaborData.frame_categories(trial, :) = frame_categories;
        %         GaborData.checksum(trial) = checksum;
        
        % Record answer of the ideal observer.
        GaborData.ideal_frame_signals(trial, :) = ideal_frame_signals;
...
%             bpg.getSignal(double(image_array) - 127, GaborData.right_category, max(GaborData.noise(trial), .04));
        ...
        if isempty(GaborData.model_observer)
            ...
            [I, tracker_info, broke_fixation, quit] = trialStimuliGabor_pre_gen(GaborData, uint8(image_array), wPtr, tracker_info, settings);
