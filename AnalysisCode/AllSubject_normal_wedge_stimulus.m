...
time(2) = 1/12;
expt_type = 2;
dir = 'RawDataNormalWedgeStimulus';
subjects = {...
    'bpgFinaltest-subject09', 'bpgFinaltestWedge-subject01';%katherine (less trials)
    'bpgFinaltest-subject18', 'bpgFinaltestWedge-subject02';%sarah
    'bpgFinaltest-subject17', 'bpgFinaltestWedge-subject03';%grace (big errorbars in PK fit)
    'bpgFinaltest-subject19', 'bpgFinaltestWedge-subject04';%gabriel
    'bpgFinaltest-subject21', 'bpgFinaltestWedge-subject05';%lucy
    'bpgFinaltest-subject20', 'bpgFinaltestWedge-subject06';%alexis (big lapse>0.2)
    'bpgFinaltest-subject22', 'bpgFinaltestWedge-subject07' %brenda
    };
subjectsID_normal = {...
    'bpgFinaltest-subject09';%katherine (less trials)
    'bpgFinaltest-subject18';%sarah
    'bpgFinaltest-subject17';%grace (big errorbars in PK fit)
    'bpgFinaltest-subject19';%gabriel
    'bpgFinaltest-subject21';%lucy
    'bpgFinaltest-subject20';%alexis (big lapse>0.2)
    'bpgFinaltest-subject22' %brenda
    };
subjectsID_wedge = {...
    'bpgFinaltestWedge-subject01';%katherine (less trials)
    'bpgFinaltestWedge-subject02';%sarah
    'bpgFinaltestWedge-subject03';%grace (big errorbars in PK fit)
    'bpgFinaltestWedge-subject04';%gabriel
    'bpgFinaltestWedge-subject05';%lucy
    'bpgFinaltestWedge-subject06';%alexis (big lapse>0.2)
    'bpgFinaltestWedge-subject07' %brenda
    };

disp('Preloading big stimulus statistics for faster post computations....');
[num_sub,~] = size(subjects);

disp('Starting to find best hyperparameters for WEDGE data across subjects....');
[best_hprs_wedge] = CustomRegression.combined_hprs_search(subjectsID_wedge, expt_type, hpr_ridge, hpr_ar1, hpr_curvature, standardize, folds, dir);
disp('Starting to find best hyperparameters for NORMAL data across subjects....');
[best_hprs_normal] = CustomRegression.combined_hprs_search(subjectsID_normal, expt_type, hpr_ridge, hpr_ar1, hpr_curvature, standardize, folds, dir);
...
for i=1:(num_sub)
    ...
    for j=1:cases
        if j==1
            disp(['Starting analysis for NORMAL data of Subject ' num2str(i) ' ...']);
            best_hprs = best_hprs_normal;
            ...
            disp(['Starting analysis for WEDGE data of Subject ' num2str(i) ' ...']);
            best_hprs = best_hprs_wedge;
            [params_boot,~,~,sobl,~,~,abbl_exp,~,~,trials,bin_centers,~,~,data,frame_signals,sobl_time_locked,~,~,log_bernoulli_sub{i,j}] = run_analysis_noise_only(subjects{i,j},expt_type,time(j),boots,best_hprs,standardize,dir);
        end
...
sgtitle('Check how good weights predict normal stimulus behavior','fontsize',30);
...
sgtitle('Check how good weights predict wedge stimulus behavior','fontsize',30);
...
for i=1:(num_sub)
    ...
    legend({['normal ' '(' num2str(num_trials(i,1)) ' trials)'],['wedge ' '(' num2str(num_trials(i,2)) ' trials)']});
    title(['Normal Vs Wedge Stimulus for Subject ' num2str(i)])
    hold('on');
    axis('tight');
end
...
for i=1:(num_sub)
    ...
    legend({['normal trials'],['wedge trials']});
    title(['Normal Vs Wedge Stimulus for Subject ' num2str(i)])
    hold('on');
    axis('tight');
end
...
hold('on');
xticklabels({'Normal Stimulus','','Wedge Stimulus'});
xlabel('Normal Stimulus (blue) to Wedge Stimulus (green)');
ylabel('Slopes');
hold('on');
...
xlabel('Slopes for Normal Stimulus ');
ylabel('Slopes for Wedge Stimulus');
hold('on');
title('Slope Comparison')
hold('on');
axis('tight')
...
xlabel('Beta for Normal Stimulus ');
ylabel('Beta for Wedge Stimulus');
hold('on');
...
title('Normal Stimulus Norm Weights')
...
title('Wedge Stimulus Norm Weights')
hold('on');
axis('tight')
...
xlabel('Norm Slopes for Normal Stimulus ');
ylabel('Norm Slopes for Wedge Stimulus');
hold('on');
title('Norm Slope Comparison')
hold('on');
axis('tight')
...
xlabel('Norm Slope for Normal Stimulus ');
ylabel('Norm Slope for Wedge Stimulus');
hold on;
...
legend ({['Normal Stim'] ['Wedge Stim']},'Box','off','Fontsize',10)
ylim([-5 5]);
xlim([1 10]);
...
scatter(sum(mn_slp1),sum(mn_slp2),200,'r','filled');
xlabel('Thresh \kappa for Normal Stim','FontSize',20);
ylabel('Thresh \kappa for Wedge Stim','FontSize',20);
hold('on');
hold('on');
axis('tight');
get(gca,'XTickLabel');
...
mn_slp2 = ((1./v2)./sum(1./v2)) .* norm_slope(:,2)';
scatter(sum(mn_slp1),sum(mn_slp2),200,'r','filled');
xlabel('Slopes for Normal Stimulus ','FontSize',20);
ylabel('Slopes for Wedge Stimulus','FontSize',20);
hold('on');
hold('on');
axis('tight')
...
axis('tight')
xlabel('Signed Kappa');
ylabel('Percent chose left for Normal Stimulus');
ax = gca;
ax.LineWidth=2;
set(ax, 'box','off');
ax.XAxis.FontSize = 20;
...
axis('tight')
xlabel('Signed Kappa');
ylabel('Percent chose left for Wedge Stimulus');
set(gca,'fontsize',18)
ax = gca;
ax.LineWidth=2;
set(ax, 'box','off');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
close all;
filename_save = strcat('SavedWorkspace/NormalWedge_',date,'.mat');
save(filename_save);
