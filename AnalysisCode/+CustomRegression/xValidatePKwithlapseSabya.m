function [best_hprs, log_likelihoods] = xValidatePKwithlapseSabya(data, responses, hpr_ridge, hpr_ar1, hpr_curvature, standardize, folds)
...
% Standardize each regressor first, across the whole dataset (this is identical to the CustomRegression.PsychophysicalKernel function)
switch standardize
    case 0
        % do nothing
    case 1
...
parfor ii=1:numel(log_likelihoods)
    ...
    [params, ~, ~, ~, ~, ~] = ...
        CustomRegression.PsychophysicalKernelwithlapseSabya(data(dataIdx, :), responses(dataIdx),  ...
        hpr_ridge(iRidge), hpr_ar1(iAR1), hpr_curvature(iCurve), standardize);
