...
time(2) = 1/12;
expt_type = 2;
dir = 'RawDataSmallBigStimulus';
subjects = {...
    'bpgshorterframes-subject02', 'bpgshorterframesbigstim-subject05';...
    'bpgshorterframes-subject04', 'bpgshorterframesbigstim-subject01';
    'bpgshorterframes-subject05', 'bpgshorterframesbigstim-subject04';
    'bpgshorterframes-subject03', 'bpgshorterframesbigstim-subject03';
    'bpgshorterframes-subject08', 'bpgshorterframesbigstim-subject06';
    'bpgshorterframes-subject09', 'bpgshorterframesbigstim-subject07';
    'bpgshorterframes-subject10', 'bpgshorterframesbigstim-subject10';
    'bpgshorterframes-subject11', 'bpgshorterframesbigstim-subject09';
    'bpgshorterframes-subject14', 'bpgshorterframesbigstim-subject08'
    };
subjectsID_small = {...
    ...
    'bpgshorterframes-subject08';
    'bpgshorterframes-subject09';
    'bpgshorterframes-subject10';
    'bpgshorterframes-subject11';
    'bpgshorterframes-subject14'
    };
subjectsID_big = {...
    'bpgshorterframesbigstim-subject05';...
    'bpgshorterframesbigstim-subject01';
    'bpgshorterframesbigstim-subject04';
    'bpgshorterframesbigstim-subject03';
    'bpgshorterframesbigstim-subject06';
    'bpgshorterframesbigstim-subject07';
    'bpgshorterframesbigstim-subject10';
    'bpgshorterframesbigstim-subject09';
    'bpgshorterframesbigstim-subject08'
    };
[num_sub,~] = size(subjects);

disp('Starting to find best hyperparameters for BIG STIMULUS data across subjects....');
[best_hprs_big] = CustomRegression.combined_hprs_search_small_big_stimulus(subjectsID_big, expt_type, hpr_ridge, hpr_ar1, hpr_curvature, standardize, folds, dir);
disp('Starting to find best hyperparameters for SMALL STIMULUS data across subjects....');
[best_hprs_small] = CustomRegression.combined_hprs_search_small_big_stimulus(subjectsID_small, expt_type, hpr_ridge, hpr_ar1, hpr_curvature, standardize, folds, dir);
...
for i=1:(num_sub)
    ...
    sgtitle(['Top Row: Small Stimuli and Bottom Row: Big Stimuli for Subject ' num2str(i)])
...
for i=1:num_sub
    ...
    hold on;
    plot(vals,squeeze(subj_resp_pred(i,j,:)),'r','Linewidth',2);
    yline(0.5,'--k');
    xline(0.0,'--k');
    xlabel('Signed Kappa');
    ylabel('Percent chose left');
...
sgtitle('Response predicted by fitted weights to real data for small stimuli','fontsize',30)
...
sgtitle('Response predicted by fitted weights to real data for big stimuli','fontsize',30)
%%
bin_num = 15;
figure();
llo_mn = [];
...
sgtitle('Check how good weights predict small stimulus behavior','fontsize',30);
...
sgtitle('Check how good weights predict big stimulus behavior','fontsize',30);
...
for i=1:(num_sub)
    ...
    legend({['small ' '(' num2str(num_trials(i,1)) ' trials)'],['big ' '(' num2str(num_trials(i,2)) ' trials)']});
    title(['Small Vs Big Stimulus for Subject ' num2str(i)])
    hold('on');
    axis('tight');
end
...
for i=1:(num_sub)
    ...
    legend({['small stimuli'],['big stimuli']});
    title(['Small Vs Big Stimulus for Subject ' num2str(i)])
    hold('on');
    axis('tight');
end
...
for i=1:(num_sub)
    ...
    legend({['small stimuli'],['big stimuli']});
    title(['Small Vs Bigl Stimulus for Subject ' num2str(i)])
    hold('on');
    axis('tight');
end
yline(0.0,'k','linewidth',2);
...
hold('on');
xticklabels({'Small Stimulus','','Big Stimulus'});
xlabel('Small Stimulus (blue) to Big Stimulus (green)');
ylabel('Slopes');
hold('on');
...
xlabel('Slopes for Small Stimulus ');
ylabel('Slopes for Big Stimulus');
hold('on');
title('Slope Comparison')
hold('on');
axis('tight')
...
xlabel('Beta for Small Stimulus ');
ylabel('Beta for Big Stimulus');
hold('on');
...
title('Small Stimulus Norm Weights')
...
title('Big Stimulus Norm Weights')
hold('on');
axis('tight')
...
xlabel('Norm Slopes for Small Stimulus ');
ylabel('Norm Slopes for Big Stimulus');
hold('on');
title('Norm Slope Comparison')
hold('on');
axis('tight')
...
xlabel('Norm Slope for Small Stimulus ');
ylabel('Norm Slope for Big Stimulus');
hold on;
...
legend ({['Small Stim'] ['Big Stim']},'Box','off','Fontsize',10)
ylim([-5 5]);
xlim([1 10]);
...
scatter(sum(mn_slp1),sum(mn_slp2),200,'r','filled');
xlabel('Thresh \kappa for Small Stim','FontSize',20);
ylabel('Thresh \kappa for Big Stim','FontSize',20);
hold('on');
hold('on');
axis('tight');
get(gca,'XTickLabel');
...
mn_slp2 = ((1./v2)./sum(1./v2)) .* norm_slope(:,2)';
scatter(sum(mn_slp1),sum(mn_slp2),200,'r','filled');
xlabel('Slopes for Small Stimulus ','FontSize',20);
ylabel('Slopes for Big Stimulus','FontSize',20);
hold('on');
hold('on');
axis('tight')
...
axis('tight')
xlabel('Signed Kappa');
ylabel('Percent chose left for Small Stimulus');
ax = gca;
ax.LineWidth=2;
set(ax, 'box','off');
ax.XAxis.FontSize = 20;
...
axis('tight')
xlabel('Signed Kappa');
ylabel('Percent chose left for Big Stimulus');
set(gca,'fontsize',18)
ax = gca;
ax.LineWidth=2;
set(ax, 'box','off');
