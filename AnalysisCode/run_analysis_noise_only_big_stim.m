function [params_boot,sobl,abbl,trials,bin_centers,means,stderrs,data,frame_signals,sobl_time_locked,log_bernoulli] = run_analysis_noise_only_big_stim(subjectID, expt_type, time, boot_n, best_hprs, correct_signals_pre, faulty_signals_pre, ratios, contrasts, standardize, dir)
...
if exist(frame_signal_filename)
    ...
    if abs(size(frame_signals,1)-length(data.choice))>0
        [frame_signals, ~] = ComputeFrameSignals_bigstim(data, correct_signals_pre, faulty_signals_pre, ratios, contrasts, trial_types);
