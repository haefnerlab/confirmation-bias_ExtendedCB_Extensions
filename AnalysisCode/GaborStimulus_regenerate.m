function [image_array, frame_categories, checksum] = GaborStimulus_regenerate(GaborData, trial, regen)
...
elseif isfield(GaborData, 'use_wedge_code') && GaborData.use_wedge_code
    stim_fcn = @bpg.genImages_wedge;
else
...
rng(GaborData.seed(trial), 'twister');
if isfield(GaborData, 'use_wedge_code') && GaborData.use_wedge_code
    image_array = stim_fcn(GaborData.number_of_images, GaborData.stim_size, ...
        ...
        frame_categories, GaborData.noise(trial), GaborData.annulus,GaborData.wedge_chosen(trial));
else
    image_array = stim_fcn(GaborData.number_of_images, GaborData.stim_size, ...
        GaborData.stim_sp_freq_cpp, GaborData.stim_std_sp_freq_cpp, ...
...
if nargin >= 3 && regen > 0
    ...
    if regen == 1
        ...
        if checksum == GaborData.checksum(trial)
            ...
            %             disp(['Issues with trial number: ' num2str(trial)]);
            %             return;
